...
Loop:
    JUMP BTIMER              ;*****This jump should still work
BRETURN:    CALL   GetError
    LOAD   AngularError
    ADDI   -90
    STORE  AngularError
    JNEG   Q1
...
; falls through from above.
    CALL   Wait1           ;***** Added this line so the robot doesn't stop before sending the last location. Might not be needed.
...
Next:
   ...
   LOAD   Four
   OUT    BEEP
   ;LOAD   ZERO  ;*****If the beep doesn't work, try removing the semicolon before load
   OUT TIMER

   LOAD   Pointer   ;*****The first x and y is set in main, so we need it's location indicated first, then update to the next x and y
   ADDI   1
   STORE  Pointer
   ILOAD  Pointer
   CALL   IndicateDest
   ...
   LOAD   Pointer
   ADDI   -38        ;*****If the robot is stopping too soon and not sending 12 lines with IndicateDest, then try increasing this value to -38 or so. This might require adding a couple "DW 0" to the end of the table.
   SUB    InitPointer
   JZERO  Die
   JPOS   Die
   JUMP   Loop
...
BTIMER: 
	 ...
	 JNEG   BRETURN
	 LOAD ZERO
	 OUT  BEEP      ; Added this line. Looks like I forgot to actually stop the beep, so this method might still work.
	 JUMP BRETURN	    
